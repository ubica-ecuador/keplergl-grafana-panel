...
copy beside the one this plugin depends on, and deck.gl refuses to start: `multiple versions
detected`. It surfaces first as jest suites that fail to load, and it is worth re-checking after
every kepler bump — the fix is to move the pins, not to remove them.
