...
| Playhead variable              | `playheadVariable`    | —                     | Writes the trip animation's current instant to a variable as UTC ISO 8601: once on load, during playback at the interval above, and where it stops. One way, independent of `timeSync`. A stat of the taxis on the road right then: `SELECT count(*) FROM trips WHERE $playhead::TIMESTAMPTZ BETWEEN trip_start AND trip_end`. Set it on one map only. |
