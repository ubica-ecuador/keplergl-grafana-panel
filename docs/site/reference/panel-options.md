...
| Raster tile server             | `rasterServerUrl`     | —          | TiTiler-compatible server that turns a COG into tiles, for queries returning a `raster_url` column. It reads the imagery on the map's behalf, so it has to be able to reach it. See [Field roles](./field-roles). |
